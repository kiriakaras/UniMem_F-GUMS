...
void copy_data_128(char* dst, const char* src, size_t size)
{
        ...
        while(size) {
        //      printf("doing OK, size is: %d\n\r", size);
                _mm128_store_si128 ((__m128i*)dst, _mm128_load_si128((__m128i const*)src));
        //                      printf("went OK\n\r");
...
int main(int argc, char** argv)
{
    ...
    char * temparray;
    temparray = (char*)aligned_alloc(64, size);
    if(mode == 1)
...
//    printf("Time elapsed in ns is: %f\n", (double)timeElapsed);
    ...
    printf("Bandwidth (MB/s) achieved is: %lf\n", (double)(  ((size2 / 1000000) / (double)( (double)timeElapsed / 1000000000))*100));
