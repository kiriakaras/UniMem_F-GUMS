...
typedef struct acc_data {
    ...
    int reg_offset[16]; // the register offsets to be written
    long int reg_data[16];       //and their data
    int poll_value;
    } acc_data;
...
int main(int argc, char** argv)
{
    ...
    for(i=0;i<16;i++){
        accel_cfg.reg_offset[i] = 0;
        accel_cfg.reg_data[i] = 0;
    }
  ...
  accel_cfg.accel_number = 0; // 0 for HX accelerator, 1 and 2 for MV accelerator
  accel_cfg.reg_number = 3;

  accel_cfg.reg_offset[0] = 0x24;
  accel_cfg.reg_data[0] = 0x10000000000;

  // setting up second register
  accel_cfg.reg_offset[1] = 0x30;
  accel_cfg.reg_data[1] = 0x1980000000;
  // setting up the output register
  accel_cfg.reg_offset[2] = 0x18;
  accel_cfg.reg_data[2] = 0x20F0000000;

  printf("%x\n",(unsigned int)(((unsigned int)(accel_cfg.reg_data[0]>>32))));
  printf("%x\n",(unsigned int)(accel_cfg.reg_data[0]));

  // Start the accelerator
  accel_cfg.reg_offset[3] = 0; // control register
  accel_cfg.reg_data[3] = 1; // write value 1 to start the accelerator
...
//    printf("Time elapsed in ns is: %f\n", (double)timeElapsed);
    ...
    if (ioctl(configfd, ACC_SETUP, (unimem_data *)&accel_cfg) == -1)
