...
#define ACC_SETUP _IOW('q', 'b', int *)
#define ACC_POLL _IOW('q', 'c', int *)
...
switch(cmd){
  ...
  //      printk("Message PASS_STRUCT base_addr : %d\n",data.base_addr);
	...
	ret = copy_from_user(&registers_vector, arg, sizeof(int)*30);
        if(ret < 0){
...
static int unimem_init(void)
{
  ...
  netdev = __dev_get_by_name(&init_net,"enp1s0"); //finding the network device to extract the MAC
